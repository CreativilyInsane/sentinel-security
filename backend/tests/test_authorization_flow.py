...
class TestModulePermissionEnforcement:
    ...
    def _make_user(self, role_name: str, *, user_id: int = 1, perms: Optional[dict] = None):
        ...
        return SimpleNamespace(
            ...
            role=role,
            is_active=True,
            # NOTE: private_scan_enabled has been removed; private-network
            # scan access is now controlled by the per-user
            # ``private_network_scan`` module permission in ``perms``.
            module_permissions=[
...
class TestAuthorizationFlow:
    ...
    def test_target_authorization_service_uses_assignments(self):
        ...
        # Step 6 — private target without assignment now consults the
        # DB-backed private_network_scan module permission (legacy
        # private_scan_enabled flag has been removed).
...
class TestReportAuthorization:
    ...
    def test_report_html_endpoint_calls_get_scan_first(self):
        ...
        Phase 19 update: the endpoint now accepts either a JWT (via
        ``get_current_user_or_none``) or a signed report token
        (``?token=`` query param).  The actual user lookup is done by
        ``_resolve_report_user`` and the ownership check is delegated
        to ``service.get_scan``.  We assert on the resolved user
        object (``user.id`` / ``_is_admin(user)``) instead of the
        ...
        assert "is_admin=_is_admin(user)" in src
